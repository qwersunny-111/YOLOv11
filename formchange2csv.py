...
# YOLO标签文件夹路径
label_folder = '/home/sunleyao/sly/ultralytics/runs/detect/predict5/labels'
    ...
    if filename.endswith('.txt'):
        ...
        with open(os.path.join(label_folder, filename), 'r') as file:
            for line in file:
                ...
                width = float(parts[3]) * img_width / 2
                height = float(parts[4]) * img_height / 2
                ...
                # 将数据添加到行列表
                data_rows.append([id_counter, image_id, class_id, x_min, y_min, int(width), int(height)])
                id_counter += 1
                row_count += 1
